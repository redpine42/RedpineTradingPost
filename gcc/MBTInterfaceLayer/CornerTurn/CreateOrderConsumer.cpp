...
CreateOrderConsumer::CreateOrderConsumer(CreateOrderInterface * cb)
	: cb_(cb),
	  latch(1),
      doneLatch(999999999)
{
...
CreateOrderConsumer::~CreateOrderConsumer(void)
{
}

void CreateOrderConsumer::waitUntilReady() 
{
    latch.await();
}

void CreateOrderConsumer::run() 
{
	try 
	{
        ...
        destination = session->createTopic( "CreateOrder" );
...
void CreateOrderConsumer::onMessage( const Message* message )
{
    try
    {
...
void CreateOrderConsumer::onException( const CMSException& ex AMQCPP_UNUSED) 
{
