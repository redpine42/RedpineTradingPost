...
L1DataConsumer::L1DataConsumer(L1DataInterface * cb)
	: cb_(cb),
	  latch(1),
      doneLatch(999999999)
{
...
L1DataConsumer::~L1DataConsumer(void)
{
}

void L1DataConsumer::waitUntilReady() 
{
    latch.await();
}

void L1DataConsumer::run() 
{
	try 
	{
...
void L1DataConsumer::onMessage( const Message* message )
{
    try
    {
...
void L1DataConsumer::onException( const CMSException& ex AMQCPP_UNUSED) 
{
